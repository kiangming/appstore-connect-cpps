...
- [ ] **[POOL-key-management-UI] — a Settings screen for pool keys.** Today the entire management surface is `scripts/seed-asc-pool-key.mjs`, which is a deliberate v1 decision (`[K1.3]`), not an oversight: pool keys must NOT live in `public.asc_accounts`, whose account picker is **shared with CPP Manager**, and `private_key_enc` must be AES-256-GCM ciphertext from the app's own routine — neither a form nor the SQL Editor can produce that without shipping a second encryption path. Scope when it happens: list / add / disable **per account**, upload the `.p8`, show `cooldown_until`. ⚠ **Opening condition: the pool has run cleanly through UAT AND D1 returned per-key.** Building a management UI for a pool that turns out to be per-team would be building a UI for a feature that stays dark forever. ⚠ Whoever builds it should read `docs/iap-management/RUNBOOK-seed-pool-keys.md` first — every manual step and every self-check in there is a requirement the UI has to absorb, especially §2's "the only place a mis-assignment can happen is the keyboard" and §4a's eyeball check, which a UI could eliminate entirely by picking the account from a dropdown instead of a typed id.
