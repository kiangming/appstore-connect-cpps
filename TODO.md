...
- [ ] **[RATELIMIT-keypool-if-demand] Key-pool rotation — CLOSED, with a stated reopen condition.** Proposal was ~10 keys per ASC account (5 accounts ⇒ ~50 `.p8` files) with 429 → rotate key. **Closed for three reasons, in order of weight:** (1) **Demand does not reach the ceiling.** Measured cap is 3,600/h; the heaviest known job — export of a 500-item app — costs **1,503** requests, so two full exports fit in one rolling hour with ~360 left over. Rotation multiplies a ceiling nothing is currently hitting. (2) **Its precondition was never measured.** Whether the 3,600 budget is per-KEY or per-TEAM is unknown — the 2026-08-25 probe used one key only. If it is per-team, rotation buys **exactly zero** and the whole design is void. The measurement is cheap and specified (census D1: 1 extra key on the same team + 11 read-only requests; `rl-lib.sh` is written) but has not been run. (3) **It is the most invasive option on the table.** It needs `asc_accounts` split into account-vs-key (a real schema change, not extra rows — otherwise the shared `AccountSwitcher` dropdown shows 50 entries to **both** CPP Manager and Apple IAP, which share `session.activeAccountId`), and it redefines the shipped stop-latch contract in 5 docstrings + ~6 tests ("a 429 that survived retry = budget exhausted" becomes "this KEY is exhausted"). ⚠ **Reopen only when BOTH hold:** (a) measured demand exceeds **3,240 req/h** (3,600 minus a 10% reserve) *after* `[RATELIMIT-demand-reduction-e3]` has shipped — not projected demand, measured from `[asc-client] … budget=` lines; and (b) census D1 has been run and returned **per-KEY**. Either one failing keeps this closed.
- [ ] **[RATELIMIT-demand-reduction-e3] Export costs 3 Apple requests per item; the price-schedule stage 1 may be foldable into the detail read.** Stage 1 (`GET /v2/inAppPurchases/{id}/iapPriceSchedule`) exists only to obtain `scheduleId` + `baseTerritory`. Apple's own OpenAPI (`docs/iap-management/openapi.oas.json`) lists **`iapPriceSchedule` in the `include` enum** for `GET /v2/inAppPurchases/{id}`, and types it `data.type: object` (**to-one**) — so §4.1's to-many relationship truncation does **not** apply. Would take a 500-item export from **1,503 → 1,003** requests (−33%). ⚠ **Blocked on one verification GET, not on opinion** — script with 4 gates in `scratchpad/P1-P3-audit-spec.md` PART P3. The decisive gate is **not** "does the include work" but "does the side-loaded schedule still carry `relationships.baseTerritory.data.id`": JSON:API gives `included[]` resources `links` only, so the base territory may vanish. That failure is now loud rather than silent (F2 shipped `baseTerritory: string | null` + the `UNKNOWN_BASE_TERRITORY` export-failure kind), which is precisely what makes attempting this safe. Run against an IAP whose base is **not** USA.
