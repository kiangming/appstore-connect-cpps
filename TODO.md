...
- [ ] **[RATELIMIT-keypool-design] Key pool — ACTIVE, design phase.** *(was `[RATELIMIT-keypool-if-demand]`, closed-with-reopen-condition; reopened 2026-08-25 by Manager decision.)* **`[Q-RATELIMIT.per-key-confirmed]` — the Manager confirms from operating a key pool on another tool that Apple's hourly limit is counted PER KEY, not per team.** That was the one precondition the census could not settle, and it was the only thing gating the design; the if-demand condition is therefore **removed**, not merely met. ⚠ Census D1 (11 read-only requests, `rl-lib.sh`, one extra key on the same team) is **not cancelled — it changes role**: from "decide whether to build this" to "confirm on THIS system, at pool-setup time, once the second key exists". Record the result in KB §4.9 alongside the `user-hour-lim` measurement. Same discipline that section already argues for: Apple's own docs said 3,600 and Hotfix 25 still shipped 250 for months because nobody read it off the wire. Operating experience from another tool is strong evidence, not a measurement of this one.
  - **`[Q-RATELIMIT.pool-scope]` — LOCKED SCOPE: the pool serves Apple IAP Management item flows ONLY.** CPP Manager keeps its existing single-key-per-account mechanism and must not observe any behavioural change. This is a hard constraint on the design, not a phasing preference — `appleFetch` is shared by both modules (`lib/shared/apple-fetch.ts:208` is the only `generateAscToken` call site in the repo), so "pool" and "not pool" have to be distinguishable at that one choke point without CPP opting in to anything.
