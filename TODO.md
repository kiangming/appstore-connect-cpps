...
- [ ] ⚠ [SC6-BLOCKER] **The bulk subset picker has no UI entry point** — `app/(dashboard)/iap-management/apps/[appId]/IapListClient.tsx:408,417` only ever set `bulkMode` to `"set-all"` / `"remove"`; nothing sets `"set-territories"`. The modal, the route schema, the orchestrator and the tests all support it (SC6p1/SC6p2), so this is one button plus its label — but until it exists, surface A of the shipped feature is **unreachable** and must not be documented or trained on as available. Found during the closing docs pass, not by any test: the same layer-gap shape as the SC6p1 route-schema find, one layer further out. Fix = add the third toolbar button next to Set Availabilities / Remove from Sales, then bring `docs/user-docs/index.html` (`apple-bulk-availabilities` section + its 2-tab SVG) to parity.
- [ ] ⚠ [SC3-consumer-gap] **`set-all` / `remove` render `NOT_ATTEMPTED` as "Failed"** — `components/iap-management/AvailabilitiesBulkModal.tsx:1155-1185` (`ProgressList`) keys off `r.ok` alone, and a never-attempted row carries `ok: false` (`lib/iap-management/orchestrators/bulk-availability.ts:339`), so it draws a red `!` + "Failed". SC3 added the third state to the SHARED orchestrator; SC6p2 gave only `set-territories` the three-state view (`BulkResultsView`) and left the legacy consumer untouched. Consequence: on a rate-limit stop the operator is told to redo work that was never sent, and cannot see that a retry is safe. Fix = route those modes through `BulkResultsView` too, or teach `ProgressList` the third state. P5 status-principle violation; the user-docs callout currently warns operators about it in prose as a stopgap.
