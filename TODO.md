...
- [ ] 🟠 **[ASC-CLIENT-outside-pool] Eleven Apple IAP Management entry points spend the ACCOUNT key, outside the pool — a 429 path completely different from the one under investigation.** Found 2026-09-04 while tracing a Railway log line, not by looking for it.
  ...
  - **Not yet known — needs the Manager, cannot be read from code:** which account/pool `WYMNPMQBT4` belongs to (it is not among the 4 keys measured for D1). SQL: `SELECT account_id, key_id, enabled FROM iap_mgmt.asc_account_keys WHERE key_id = 'WYMNPMQBT4';` — **KỲ VỌNG** one row naming the account; **zero rows** would mean it is an `asc_accounts.key_id` instead, and then `pool=` on that line was reporting a non-pool key, which would be worth a second look.
