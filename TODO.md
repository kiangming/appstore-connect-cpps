...
- [ ] [LOC-ACTIVE-ui-warning] ⏸ **Cảnh báo UI cho ô ACTIVE** — Manager chốt (A)+(C) 2026-09-23: vẫn gửi mọi `toPatch` (Apple là trọng tài, KHÔNG skip), **nhưng** cảnh báo ở step Localization cho **ĐÚNG MỘT giá trị ĐÃ ĐO**: `ACTIVE` ⇒ *"ô này nhiều khả năng bị Apple từ chối (đang ACTIVE — cần tạo version mới)"*. ⚠ **KHÔNG cảnh báo cho giá trị CHƯA BIẾT** (`APPROVED` · `WAITING_FOR_REVIEW` · `REJECTED`) — cảnh báo sai còn tệ hơn không cảnh báo. ⏳ **CHẶN BỞI PROBE**: không có `state` thì không có gì để cảnh báo. Bảng 5 giá trị: KB §30.1.
...
## Export picker census (Apple) — found 2026-09-03, arc `arc-apple-export-picker-paging`

Found while censusing the Export-list item picker for pagination + shift-click
range (`docs/iap-management/design-export-picker-paging-range.md`). NOT folded
into that arc — the arc is design-only and awaiting Manager sign-off.

- [ ] [Y-aprime-paged] `BulkItemPicker`'s `paged` prop is **OFF for A′** (`components/iap-management/AvailabilitiesBulkModal.tsx:1081` passes it not at all; `components/iap-management/item-picker/BulkItemPicker.tsx` defaults it to `false`), so the availability bulk modal keeps the window + "Show more" it has today and gets **neither** shift-click range selection nor (from Y2) pagination. ⚠ **THIS IS Q2, A MANAGER DECISION, NOT AN OVERSIGHT** — export is a READ path where a wrong pick costs a re-run, A′ is a WRITE path where a wrong pick has already changed which territories an item sells in and re-running does not undo it. The risk is not symmetric, so one UI refactor was not allowed to drag both surfaces along at once. Turning it on for A′ is a **separate decision, after the new picker has run in production on export** — and it needs its own UAT, because A′'s bulk controls write to Apple. ⚠ The prop is **not** dead code: both branches are pinned in `components/iap-management/item-picker/BulkItemPicker.range.test.tsx` ("⚠ DEFAULTS to off …"), and flipping the default to `true` turns that test red. ⓘ Measured during the Y1 gauntlet: A′'s own 77 tests stay **GREEN** under that default flip — they never exercise the gesture — which is exactly why the gate is asserted at the shared layer and not left to A′'s suite.
- [x] ~~[EXPORT-wizard-reopen-lands-on-step-2]~~ ✅ **FIXED 2026-09-04 (arc `arc-apple-export-picker-paging`, chunk Y3).** Reset is keyed on the dialog **OPENING**, not on each close path — `useEffect(() => { if (open) reset(); }, [open])` in `components/iap-management/export-wizard/ExportItemWizard.tsx`. ⚠ **That choice is the fix, not a detail.** Patching the one broken exit (`handleConfirmExport`) would have left the ✕/Cancel paths each carrying their own copy of the reset, and a third exit added later — an Esc handler, a route change, a parent remount — would have reproduced this defect identically. Keying off the transition INTO open covers every exit that exists and every one not yet written. ⚠ **The gap was the ASSERTION, not the logic**: `grep -i reopen` over the wizard's test file returned nothing, because every test opened it once and drove forward. `IapListClient.export-reopen.test.tsx` (6 tests) now performs the gesture — both exits, plus a zero-fetch guard; deleting the effect turns 2 of them red. ⓘ Original entry below.
  ⓘ ~~After a successful export, re-opening "Export list" renders **step 2 (countries)** with the previous selection still ticked — the item-selection step is skipped entirely. `components/iap-management/export-wizard/ExportItemWizard.tsx` has **zero `useEffect`** (so no reset-on-open) and is **permanently mounted** (`open` is a prop, `IapListClient.tsx:1036`); `handleConfirmExport` only calls `setExportWizardOpen(false)` (`IapListClient.tsx:427`) and never the wizard's own `reset()` (`ExportItemWizard.tsx:285-293`), so `step` is still `"countries"` from `:550`. ⚠ Not a data defect — the selection is intact and the route contract is untouched — but the Manager cannot see or change what they are about to pay ~3 requests/item for. Fix is `reset()` in `handleConfirmExport`, or a reset-on-open effect; ~3 lines. ⚠ **No test covers reopen at all** (`grep -i reopen` on `IapListClient.export-wizard.test.tsx`: no match), which is why it survived — the gap is the assertion, not the logic. Raised as Q6 of the paging design; fold into that arc if the Manager approves it there.~~
...
- [x] ~~[GOOGLE-export-intersection-silent-drop]~~ ✅ **FIXED 2026-09-01 (arc G-EXPORT, chunk X4, `0955ff1`).** The selection IS the column set: `territories = [...new Set(selectedTerritories)].sort()`. A ticked country nobody prices now gets a column whose cells read `—`, meaning "this item has no price in this market". ⚠ **X4 could not have shipped without it, and that is the reason it finally got done** — X4 widened the picker from the borrowed 183 to Google's real 173, which put 15 previously-untickable markets in reach (Russia among them), i.e. exactly the ones most likely to have no price yet. Widening the list while still intersecting would have made this defect fire MORE often, so the two shipped together. ⚠ **ONE marker, not Apple's two** — Google's export makes a single list call, so a missing cell has exactly one meaning and there is no "could not check" state to distinguish it from (KB P37). Pinned by `xlsx-export.test.ts`; the test that used to assert the bug (`→ no column, no crash`) was inverted, not deleted. ⓘ Original entry below.
  ⓘ ~~[GOOGLE-export-intersection-silent-drop] **Google's export still uses the INTERSECTION that E2 removed from Apple** — `lib/google-iap-management/xlsx-export.ts:126-128`: `territories = selection ? allTerritories.filter(t => selection.has(t)) : allTerritories`. A region the operator ticks that no exported product has a price for **produces no column at all** — no `—`, no note, nothing saying the question was dropped. Apple had exactly this bug and fixed it in E2 ("the selection IS the column set"); the Google twin was never ported. ⚠ **Becomes worse the moment `[GOOGLE-regions-unmeasured]` finds unsupported codes**, since those are precisely the ticks that would vanish. Fix is E2's, one file over.~~ **← done; the prediction held exactly: `[GOOGLE-regions-unmeasured]` measured 173, and the 15 newly-tickable markets were the ones at risk.**
...
- [ ] 🟠 **[ASC-CLIENT-outside-pool] Eleven Apple IAP Management entry points spend the ACCOUNT key, outside the pool — a 429 path completely different from the one under investigation.** Found 2026-09-04 while tracing a Railway log line, not by looking for it.
  - **Mechanism.** They import `@/lib/asc-client`, whose `ascFetch` (`lib/asc-client.ts:57-64`) calls `appleFetch(..., "asc-client")` with **no `keyPool`**, so they sign with `public.asc_accounts.key_id` — never a pool key. Full list in `apple/fetch.ts`'s corrected docstring; the notable one is `app/(dashboard)/iap-management/apps/[appId]/page.tsx:81` — **the IAP list page, on every load**.
  - ⚠ **The `apple/fetch.ts` comment claimed the opposite and has been corrected** (N1): it said *"every Apple call made by IAP Management routes through this function"*. Anyone who believed it would conclude the pool protects the whole module.
  - **Measured, one Railway sample:** account key `F28D5J857Z` at `budget=3459/3600` — **141 already spent** — while pool key `WYMNPMQBT4` read `3599/3600`. A 632-item export costs ≈2,528 requests, so 141 + 2,528 ≈ **2,669 / 3,600**: fine alone, not fine if page loads have been accumulating.
  - 🔴 **THE TWO PATHS MEET AT THE FALLBACK.** `selectKey` returns the ACCOUNT's credentials when the pool is `empty` (`selector.ts:198-200`) or unreadable (`error`, `:190-196`). **An account with no pool keys is the NORMAL state** — the pool is opt-in — so such an account runs its whole export on the same key those eleven entry points are spending, and the pool cannot warn: it reads only `iap_mgmt.asc_account_keys` (`repository.ts:141-146`) and never learns what is left of the account key.
  - ⚠⚠ **NO EVIDENCE THIS CAUSED THE 2026-09-04 INCIDENT. DO NOT MERGE THE TWO.** That export was demonstrably on a pool key — `pool=key` with `68J8RLGLS9` in the log. This is a **candidate cause for FUTURE 429s that the pool cannot explain**, which is a different and so-far hypothetical problem. Diagnosing them together is how the first round of this incident went wrong.
  - ✅ **CLOSED 2026-09-04 — `WYMNPMQBT4` is a POOL key of `vng-corp`, `enabled = true` (SQL run by the Manager; do not re-run).** ⇒ **`pool=` labels correctly: no non-pool key was ever tagged as pooled**, which was the one way N2's new field could have been misleading. It also cross-checks Q1 — `WYMNPMQBT4` is one of the 7 `vng-corp` keys parked in the 394ms burst, and it is now serving normally at `budget=3599/3600`, i.e. the pool recovered exactly as the 1-hour cooldown predicted.
  - **Fix shapes, none chosen:** route the eleven through `iapFetch` (biggest blast radius, and CPP shares `asc-client`); or give `ascFetch` its own opt-in pool argument; or leave it and simply document + monitor via the `pool=` field, now on both log lines (N2). ⚠ The cheapest real mitigation is unrelated to keys: the IAP list page spends one request per load on `getApp` and could cache it.

- [ ] 🔴 **[POOL-exhaustion-reported-as-apple-429] A pool that ran out of eligible keys is reported EVERYWHERE as "Apple rate-limited us" — and it destroyed the evidence in its own investigation.** Second defect, **independent of the cooldown misattribution** and separately fixable.
  - **Root.** `ApplePoolExhaustedError extends AppleRateLimitError` (`lib/shared/apple-fetch.ts:94`), and **nothing outside that file ever checks for it** — `grep -rn ApplePoolExhaustedError --include=*.ts` returns only the definition site and the one `throw` (`:384`). So all 8 consumers treat an internal state as Apple's verdict.
  - ⚠ **The inheritance is DELIBERATE and its stated reason is now MEASURED FALSE.** The docstring (`:69-74`) justifies it as *"A pool with nothing left to hand out is the same fact those predicates exist to catch — **the budget is gone**"*. On 2026-09-04 the pool had nothing left to hand out while Apple's budget was **full** (`rem=3599 lim=3600` on both keys). The premise the inheritance rests on does not hold.
  - **The 8 sites, split by what they do — STOPPING is fine, REPORTING is not:**
    - ✅ **Correct (stop dispatching).** Nothing can be sent, so stopping is right either way: `export-fetch.ts:198` · `bulk-availability.ts:347` · `availability-sweep.ts:183` · `submit-batch/route.ts:595`.
    - ❌ **Wrong (attributes it to Apple).**
      - `export-fetch.ts:102-103` → `{ kind: "RATE_LIMITED", status: 429, message: errMsg(err) }` ⇒ the **"Export Failures" sheet** shows `429: All ASC pool keys … are cooling down` under the label `"Rate limited"` (`xlsx-export.ts:365`). ⚠ **This is the exact string the Manager read believing it was Apple's 429 body.**
      - `ExportResultSummary.tsx:135` → *"Export stopped — **Apple's rate limit ran out** after N of M items"* ⇒ the on-screen sentence is false in this incident.
      - `price-schedules.ts:111` → `"RATE_LIMITED"`.
      - `app/api/iap-management/iaps/[iapId]/availability/route.ts:127-129` → HTTP `{ error: "rate_limited" }` to the browser.
      - `bulk-availability.ts:442-446` → `console.warn("STOP — **Apple rate limit exhausted** …")`.
      - `availability-sweep.ts:199-202` → same wording.
      - `retry-counters.ts:62-64 + 76` → ⚠ **inflates the metrics**: `onRetry` fires 3× on the 0ms curve, so `rate429_count += 3` and `exhausted = true` **with ZERO requests sent to Apple**; that feeds `rows_throttled` (`bulk-availability.ts:499`) and the `throttled=X/Y retries=Z` audit line (`:506`).
  - ⚠ **WHY THIS IS URGENT BEYOND COSMETICS: IT BLINDS THE NEXT INVESTIGATION.** Apple's real 429 body was already being dropped (fixed as #4 in `d6cff65`), and this defect then *substituted a 429-shaped string of our own* in the one column an operator reads. The investigation spent a full round on a message the code wrote about itself. Any future rate-limit question hits the same wall, plus `rate429_count` is now a metric that counts events that never happened.
  - **Fix shape (not chosen):** the split above is the design — keep satisfying the stop predicates, stop claiming Apple said anything. Candidates: a distinct `kind: "POOL_EXHAUSTED"` in `ExportFailureKind` + its own sheet label and summary sentence; an `instanceof ApplePoolExhaustedError` guard at the 6 reporting sites; and excluding it from `onRetry` counting. ⚠ Do NOT "fix" it by removing the inheritance — that silently un-latches 4 stop predicates and their tests.
  - ⓘ Related but distinct from `[POOL-cooldown-misattribution]`: that one is about **when a key gets parked**, this one is about **what we say when the pool has no key left**. Fixing either does not fix the other.

- [ ] **[POOL-report-citation-CORRECTION] ⚠ SECOND phantom citation in the same incident — a report claimed `bulk-availability.ts:322` and `availability-sweep.ts:64` declare `CONCURRENCY = 4`. Both are FALSE and the value 4 does not exist.** Checked on `f62873d` with a clean tree:
  - `sed -n '322p' lib/iap-management/orchestrators/bulk-availability.ts` → `*                 stay fail-soft (Q-K). An exhausted budget is the one` — **prose inside a docstring**, not a declaration.
  - `sed -n '64p' lib/iap-management/apple/availability-sweep.ts` → `import {` — **an import statement**.
  - `grep -rn "CONCURRENCY \s*=\s*4\|concurrency:\s*4" --include=*.ts lib/ app/` → **empty. No concurrency of 4 exists anywhere in the repo.**
  - **The real distribution (all Apple paths):** `availability-sweep.ts:78`=2 · `bulk-availability.ts:93`=2 · `submit-batch/route.ts:101`=2 · `bulk-import/execute/route.ts:128`=2 · `availability-read-phase.ts:92`=3 · `export-fetch.ts:87`=3. Google is 5 on all five of its paths — **different API, different per-request cost, not a comparison basis for Apple.**
  - ⚠ **Why this one mattered:** chunk 1's whole justification for picking 3 is *"it is the number this module already uses"*. Had the sibling paths really been 4, that argument would have been **wrong** and the comment would have had to be rewritten. The arbitration confirmed 2·2·2·2·3·3, so the argument stands and is stronger than first written — but it was **one grep away from shipping a false rationale**.
  - ⓘ **Same family as `[POOL-cooldown-60s-claim-CORRECTION]`** (which cited `store.ts:118`, a file that does not exist). Two phantom citations in one incident ⇒ promoted to a KB rule, see KB §9 P44.

- [ ] **[POOL-cooldown-60s-claim-CORRECTION] ⚠ A PRIOR REPORT'S "cooldown = 60s hard floor, `store.ts:118`" IS WRONG ON BOTH HALVES — do not reuse that number or that citation.** Measured and re-verified 2026-09-04 on HEAD:
  - **The cooldown is ONE HOUR.** `const DEFAULT_COOLDOWN_MS = 60 * 60 * 1000;` — `lib/iap-management/key-pool/selector.ts:53`, confirmed via `git show HEAD:` with a clean worktree. It is the only cooldown constant in the module; `grep -rn "60 \* 1000\|60_000\|60000" lib/iap-management/key-pool/ lib/shared/apple-fetch.ts` finds **no 60-second value at all**.
  - **`store.ts` does not exist.** `find . -name store.ts` (excluding `.next`) returns **nothing**. The citation pointed at a file that is not in the repo.
  - **Confirmed independently by the incident's own timestamps:** `cooldown_until = 08:56:47` and the 429 burst at `~07:56:47` — a 60-minute gap, not 60 seconds. Manager's Q0 also showed the 7 `vng-corp` keys at `con_lai = -01:20:13`, i.e. expired well over an hour later.
  - ⇒ Any reasoning built on "the cooldown is only a minute, so a wrong cooldown is cheap" is invalid: a misattributed cooldown costs a **full hour** of that key, which is why the incident locked the tool out rather than briefly stuttering.

- [ ] 🔴 **[POOL-cooldown-misattribution] EVERY 429 parks a pool key for an HOUR, whether or not it was a budget refusal — and on the export path it never can be.** Incident 2026-09-04, `vng-corp`: **7 keys parked in 394ms** while Apple's budget was **untouched** (`Test key` on two keys: `rem=3599 lim=3600` on both). Every later export then died instantly on `ApplePoolExhaustedError`, so the pool locked out the tool it exists to speed up.
  - ⭐ **DATA 2026-09-04 THAT REFUTED HALF THE FIRST DIAGNOSIS — read this before reasoning about the cause.** The failing run selected **632 items, completed 556, failed 76.** The first write-up here said *"the whole export dies on the first burst"*; that is **WRONG**, and the correction matters:
    - **Concurrency-8 is NOT the trigger.** 556 items (≈2,224 requests at ~4/item) went through at width 8 with no 429. A limit hit at 8 in-flight would have killed item **one**.
    - **Concurrency-8 IS the amplifier, and that role is established.** 8 workers select 8 keys by round-robin (`selector.ts:211-212`), so ONE 429 arriving with 8 requests in flight parks **7 distinct keys in 394ms** — measured — and the 1-hour cooldown then refuses everything. At width 3 the same single 429 parks at most 3. **Two different roles; do not conflate them again.**
    - **Hourly budget: RULED OUT, twice over.** (a) Arithmetic — 2,224 requests over 7 keys ≈ **318/key** against a measured 3,600/hour. (b) Direct reading — two keys showed `rem=3599 lim=3600` at **15:31:31**, which is **34m42s after the 429 at 14:56:49**, i.e. still **inside** Apple's rolling hour, so the budget genuinely was never spent (not "recovered").
    - 🔴 **THE LIVE HYPOTHESIS IS A WINDOW SHORTER THAN AN HOUR** — per-minute, per-second, or a burst/concurrent cap. **There is NO evidence for it yet.** It needs Apple's 429 body (`RUNBOOK-pool-429-capture.md`). Do not act on it.
  - ⚠ **TIMEZONE, so nobody compares two clocks again.** The Q0 query returned `cooldown_until = 08:56:47` (**UTC**) for an incident the Manager observed at **14:56:49 local**. `cooldown_until` is `TIMESTAMPTZ` and Supabase renders UTC; Railway log timestamps are ISO-UTC too. When checking "did the 1-hour cooldown line up", convert first — the earlier round nearly read the offset as evidence about the cooldown length.
  - **Mechanism, fully traced.** `appleFetch`'s 429 branch calls `onRateLimited` whenever `selection.fromPool`, and **never reads `budget`** — `grep budget` over lines 424-476 of `lib/shared/apple-fetch.ts` returns only comments. `parseRetryAfter` → `null` (header absent) → `cooldownDurationMs(null)` → `DEFAULT_COOLDOWN_MS` = **1 hour** (`selector.ts:53`). ⇒ every 429 is treated as "hourly budget exhausted".
  - **Why 7 keys at once.** `EXPORT_FETCH_CONCURRENCY = 8` (`export-fetch.ts:50`) → `withConcurrency` runs 8 workers (`concurrency.ts:31-33`) → 8 concurrent `selectKey` calls → round-robin hands out all 7 keys (`selector.ts:211-212`, cursor increment is atomic — no `await` between read and write) → 8 concurrent 429s → 7 distinct keys parked. **394ms is the signature of ONE concurrency-8 burst, not 7 independent events.** After it, `eligible.length === 0` → `ApplePoolExhaustedError` thrown *before sending*, which is why everything then stops instantly.
  - ⚠ **ROTATION CANNOT HELP IF THE 429 IS NOT BUDGET-SHAPED.** There is no backoff *between* the 8 parallel workers (only between retries of one worker), so all 8 hit the same wall simultaneously on 8 different keys. **More keys make this worse, not better** — a bigger pool means a bigger burst parks more of it.
  - ⚠ **AND `x-rate-limit` WILL NEVER ARRIVE HERE** to fix it: KB §4.9 has now measured it absent on all three IAP endpoints, including `automaticPrices`. So "only cool down when the header confirms exhaustion" would make cooldown **dead code on the export path** — the obvious fix is not available.
  - 🛑 **DO NOT PICK A FIX BEFORE READING APPLE'S 429 BODY.** Attributing this 429 to a cause we have not read is the exact mistake that produced the incident. As of #4 the body is logged (`body=` on the rate-limited line); it is also already in the **"Export Failures"** sheet of any failed export via `AppleApiError.message` → `export-fetch.ts:162`. Repo contains **no** Apple 429 error-code taxonomy (`grep RATE_LIMIT_EXCEEDED|TOO_MANY_REQUESTS|errors[].code` → empty), so the body is the only evidence.
  - **Three candidate fixes, assessed 2026-09-04, NONE chosen:**
    - **(A) Lower concurrency / add pacing.** `EXPORT_FETCH_CONCURRENCY` 8 → 2-3 is **one constant, instantly reversible**; the full version is KB §4.9's never-built "Phase B" token bucket. ✅ Attacks the cause *if* the 429 is concurrency-shaped. ❌ Export ~2.7× slower at 8→3; and the shape is still unconfirmed.
    - **(B) Short cooldown when the header is absent** (30-60s) and one hour only when confirmed. ✅ Ends the one-hour self-lock, keeps rotation. ❌ A burst still parks all keys, just briefly. Does not address the cause.
    - **(C) N-strikes cooldown** — park a key only after ≥N 429s in X seconds. ✅ A single burst condemns nothing, and it distinguishes "transient burst" from "key really spent" **without needing a header**. ❌ More in-memory state (lost on restart); N and X are two more numbers that need measuring.
  - **Agent's non-binding read:** (A-light) + (B) — (A-light) attacks the suspected cause in one reversible line, (B) makes any remaining misattribution cost a minute instead of an hour. (C) is conceptually the cleanest and the most new machinery. ⚠ **Decide after the body is read, not before.**
  - **Unblocking meanwhile:** Settings → API Key Pool → **Clear cooldown** (shipped in #5) clears both the row and the serving instance's in-memory Map. ⚠ It is a stopgap — the next export re-parks the pool while this item is open.
  - 📓 **RUN LEDGER SINCE CHUNK 1 (width 8 → 3, live on `main` at `f62873d`).** Fill one row per Apple run; the point is to see whether 429s stop at the lower width.

    | date | action | items | 429? | notes |
    |---|---|---|---|---|
    | 2026-09-04 | export (pre-chunk-1, width 8) | 632 | **YES** — 556 ok / 76 failed | the incident |
    | 2026-09-04 | Refresh from Apple (post-deploy) | small | **no** | 3 log lines, all `→ 200`; pool live (`pool=key`) |

    ⚠⚠ **HOW TO READ THIS LEDGER, AND IT IS THE WHOLE REASON IT HAS A WARNING:
    N CLEAN RUNS DO NOT PROVE THE CAUSE IS FIXED.** Chunk 1 lowered the width;
    it did **not** touch the cooldown misattribution, and the 429's cause is
    still unread. Clean runs are consistent with "the shorter-window hypothesis
    is right and we are now under the threshold" **and equally** with "we have
    not happened to hit it again". Only Apple's 429 body distinguishes them.
    ⇒ **A clean ledger is a reason to keep the width at 3, never a reason to
    close this item or to raise the width back.**
- [ ] **[RATELIMIT-keypool-design] Key pool — 🌑 SHIPPED DARK.** Code is on `main` (K1 `77b568d` · K2 `981f408` · K3 `9322a4e`) and **inert**: `iap_mgmt.asc_account_keys` holds zero rows, so every account takes the `empty` fallback and signs with its own key exactly as before. Architecture + the two meta-rules it earned: **KB §15**. Activating it means seeding a key, not deploying anything.
  - ✅ **D1 ANSWERED 2026-09-04 — PER-KEY, measured on THIS system.** Two `Test key` clicks seconds apart on `vng-corp` both returned `rem=3599 lim=3600`; per-team would have shown 3598 on the second. Recorded in KB §4.9. ⇒ **the condition this item waited on since 2026-08-26 is satisfied, and the "if D1 returns per-team, leave the pool dark permanently" branch does NOT apply.** Extra keys carry extra budget. ⚠ **But the pool is not yet usefully live**: `[POOL-cooldown-misattribution]` (above) has it parking itself on every export. Per-key budget was necessary, not sufficient.
  - ⏳ **SUPERSEDED (was: CURRENT STATUS 2026-08-26): waiting on the Manager to add a real key.** The pool is still dark — `iap_mgmt.asc_account_keys` is empty — but the seeding path is now a UI, not a script (KB §15.4). Next step is entirely the Manager's: Settings → API Key Pool → Add key for one account (twice), then **Test key on each within a few seconds** and read the two `[key-pool-test]` lines for the D1 verdict. No script to run, no census file (the one this item used to cite no longer exists), no env to touch.
