...
vi.mock("@/lib/iap-management/hub-tracking/config", () => ({
...
vi.mock("@/lib/iap-management/hub-tracking/hub-client", () => ({ hubValidateCredentials }));

import { GET, POST } from "./route";
import { IapForbiddenError, IapUnauthorizedError } from "@/lib/iap-management/auth";
...
describe("GET /api/iap-management/hub-tracking/config", () => {
  it("401 when unauthenticated", async () => {
    requireIapAdmin.mockRejectedValue(new IapUnauthorizedError());
    const res = await GET();
    expect(res.status).toBe(401);
  });

  it("403 when authenticated but not admin", async () => {
    requireIapAdmin.mockRejectedValue(new IapForbiddenError());
    const res = await GET();
    expect(res.status).toBe(403);
  });

  it("200 with the public config shape (never a token field) for an admin", async () => {
    requireIapAdmin.mockResolvedValue(adminSession);
    const res = await GET();
...
describe("POST /api/iap-management/hub-tracking/config — save-time validation never blocks save", () => {
  beforeEach(() => {
    requireIapAdmin.mockResolvedValue(adminSession);
  });
