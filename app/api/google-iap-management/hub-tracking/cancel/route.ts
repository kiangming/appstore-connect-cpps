 ...
 *
 * Closes an in-flight Hub run as CANCELLED. Shared by every Google IAP
 * feature that tracks a run: Bulk Import (an explicit back-out, or a
 * `beforeunload` beacon) and Bulk Activate/Deactivate (declining the
 * reconfirm dialog, closing the modal before the write commits, or the
 * same `beforeunload` beacon). Called two ways: (a) a normal fetch with a
 * JSON body; (b) a best-effort `navigator.sendBeacon` on `beforeunload` —
 * sendBeacon can't set a custom Authorization header, so the token stays
 * server-side and the beacon just carries `{ run_id }` same-origin
 * (session cookie rides along automatically). Not relied on for hard
 * crashes/force-quit.
 *
