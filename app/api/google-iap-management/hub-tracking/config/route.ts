/**
 * GET/POST /api/google-iap-management/hub-tracking/config
 *
 * Settings-tier surface for the VNGGames Hub tracking integration —
 * admin-only. GET never returns the token, only
...
} from "@/lib/google-iap-management/hub-tracking/config";
import { hubValidateCredentials } from "@/lib/google-iap-management/hub-tracking/hub-client";
...
export async function POST(req: Request) {
  let session;
  try {
    session = await requireGoogleIapAdmin();
  } catch (err) {
    return authErrorResponse(err);
  }
