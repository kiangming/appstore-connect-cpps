...
export async function POST(
  ...
  try {
    ...
    await log("localization", `[API] Localization created cppId=${params.cppId} locale=${locale}`);
    return NextResponse.json(data, { status: 201 });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Internal server error";
    await log("localization", `[API] POST /api/asc/cpps/${params.cppId}/localizations error: ${message}`, "ERROR");
