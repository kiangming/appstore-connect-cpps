...
import type { HubTrackingConfigPublic } from "@/lib/cpp-hub-tracking/config";
...
export function HubTrackingClient({ initialConfig }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch("/api/asc/hub-tracking/config", {
        method: "POST",
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            Tracks each CPP Bulk Import run on the VNGGames Hub dashboard —
            started when the folder finishes loading into the tool, closed
            once the Apple upload finishes.
          </p>
        </div>
        <Link
          href="/settings"
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. cpp-bulk-import"
            className="w-full px-3 py-2 text-sm border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 rounded-lg"
          />
