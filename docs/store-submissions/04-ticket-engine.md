...
> **Implementation Note (2026-04-23, PR-9 shipped)**: Spec uses Prisma-flavored syntax (`db.$transaction`, `tx.$queryRaw`) for illustration. Actual implementation uses Supabase JS + the PL/pgSQL RPC `store_mgmt.find_or_create_ticket_tx` (migration `20260423000000_store_mgmt_ticket_engine_rpc.sql`). Semantic intent (transaction boundary, `FOR UPDATE` lock, partial unique index as race fallback) preserved; syntax adapted. Code examples in §1–§13 describe intent — the RPC + `lib/store-submissions/tickets/engine.ts` are canonical. §0 below notes shipped scope vs future work (user actions, app rename, reclassify — tracked for PR-10+).

---

## 0. Implementation status (read first)

This document specifies the full Ticket Engine — email handling with `FOR UPDATE` locks, state machine, event log, user actions, app rename. **Email handling + state machine + event log shipped in PR-9 (2026-04-23). User actions, app rename, reclassify are PR-10+.**

### PR-8 (shipped 2026-04-22, stub gap resolved by PR-9)

- `lib/store-submissions/tickets/wire.ts` — `associateEmailWithTicket(emailMessageId, classification)`
- ~~`lib/store-submissions/tickets/engine-stub.ts`~~ (deleted in PR-9.3)
...
### PR-9 (shipped 2026-04-23) — real engine

Replaced the stub with a transactional find-or-create + state machine + event log, implemented as:

- `lib/store-submissions/tickets/engine.ts` — thin TypeScript wrapper delegating to the RPC + mapping `PostgrestError` prefixes to typed errors (`TicketEngineNotApplicableError`, `TicketEngineValidationError`, `TicketEngineNotFoundError`, `TicketEngineRaceError`).
- `supabase/migrations/20260423000000_store_mgmt_ticket_engine_rpc.sql` — PL/pgSQL RPC `store_mgmt.find_or_create_ticket_tx(p_classification JSONB, p_email_message_id UUID) RETURNS JSONB`. Canonical source of truth for:
  - §3.1 transactional flow (single RPC body; equivalent to spec's `db.$transaction`)
  - §3.2 `SELECT ... FOR UPDATE` on grouping key + race retry loop catching `unique_violation` from the partial unique index
  - §3.3–3.4 create/update, `submission_id` array append, `type_payload` novelty
  - §4.1 state machine (`deriveStateFromEmailOnOpenTicket` inlined — NEW stays NEW, IN_REVIEW/REJECTED follow email outcome, terminal states unreachable via partial index)
  - §6 `ticket_entries` writes: EMAIL (idempotent via partial unique index on `(ticket_id, email_message_id) WHERE entry_type='EMAIL'`) + STATE_CHANGE on transition + PAYLOAD_ADDED on novel non-empty type_payload
- `supabase/migrations/20260423100000_store_mgmt_backfill_ticket_id.sql` — one-shot backfill for PR-8-era `ticket_id IS NULL` rows. Per-row savepoint isolation.
