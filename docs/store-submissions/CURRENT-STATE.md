...
>
> Last updated: 2026-04-23 (PR-9 planning)
...
| PR-8 | Ticket engine **stub** + wire (sync → wire → stub findOrCreateTicket) | ✅ shipped |
| PR-9 | Ticket engine **real** (RPC find_or_create_ticket_tx, FOR UPDATE, state machine, ticket_entries writes, backfill) | 🟡 planning |
| PR-10 | Inbox UI + ticket detail view | ⏳ pending |
...
| Ticket engine **stub** (ephemeral UUID, no persist) | — | `lib/store-submissions/tickets/engine-stub.ts` |
| Classification status mapping (CLAUDE.md invariant #8) | — | `lib/store-submissions/classifier/types.ts` |

---

## PR-8 known limitation — `ticket_id = NULL` cho ticketable rows

**Status**: Intended interim behavior, resolved by PR-9.

### Reason

PR-8 stub (`engine-stub.ts`) generates ephemeral UUIDs via `randomUUID()` nhưng **không INSERT vào `store_mgmt.tickets`**. FK constraint `email_messages.ticket_id REFERENCES store_mgmt.tickets(id)` (schema line 306) reject UPDATE. Wire catches error, logs `[tickets-wire] UPDATE email_messages.ticket_id failed`, và returns `null` (graceful degradation contract).

### Impact

Rows processed giữa PR-8 deploy (2026-04-22 ~10:30 UTC) và PR-9 ship có:

- `classification_status IN ('CLASSIFIED', 'UNCLASSIFIED_APP', 'UNCLASSIFIED_TYPE')`
- `ticket_id IS NULL`

Sample evidence (post-PR-8 deploy):

- 3 × UNCLASSIFIED_APP rows (13:19, 13:19, 17:08 UTC) — sender/subject matched, `extracted_app_name` populated, `app_id = NULL` (Crossfire: Legends, Total Football VNG chưa trong App Registry), `ticket_id = NULL`

### Recovery (PR-9 scope)

Backfill migration (sub-chunk 9.6) re-runs ticket association:

```sql
-- Pseudo-code
SELECT id, classification_result FROM store_mgmt.email_messages
WHERE classification_status IN ('CLASSIFIED', 'UNCLASSIFIED_APP', 'UNCLASSIFIED_TYPE')
  AND ticket_id IS NULL;
-- For each: call find_or_create_ticket_tx RPC, UPDATE email_messages.ticket_id
```

Idempotent via partial unique index `ticket_entries_email_idempotency_idx` (ditto PR-9). Resumable — only processes rows còn `ticket_id IS NULL`.

### Test coverage gap

PR-8 unit tests mock Supabase UPDATE success, không enforce FK constraint. Future harness enhancement: simulate FK hoặc integration test với real Supabase local.

---

## PR-9 preview — Ticket Engine real implementation

### Scope

- **Supabase RPC `find_or_create_ticket_tx`** (PL/pgSQL) wrapping SELECT FOR UPDATE → INSERT ticket → INSERT ticket_entries atomically
- **State machine** per spec §4.1 (NEW stays NEW; IN_REVIEW/REJECTED auto-transition on email outcome; terminal states never auto-reopen)
- **ticket_entries writes**: EMAIL entry per processed email + STATE_CHANGE on transition + PAYLOAD_ADDED on novel submission_id
- **Idempotency guard**: partial unique index `UNIQUE(ticket_id, email_message_id) WHERE entry_type='EMAIL'` — prevent double-insert on sync retry
- **Extended output fields**: `previous_state`, `state_changed`, full `ticket` row (PR-8 callers unaffected — optional additions)
- **Backfill migration** for PR-8-era NULL `ticket_id` rows

### Sub-chunks breakdown

| # | Scope | Effort |
|---|---|---|
| 9.1 | Doc update + types.ts extension (`FindOrCreateTicketOutput`) | 20m |
| 9.2 | Migration: RPC + idempotency partial index | 1.5h |
| 9.3 | `engine.ts` real impl + stub test rewrite | 2h |
| 9.4 | Wire integration test (no interface change) | 30m |
| 9.5 | State machine transition test cases | 1.5h |
| 9.6 | Backfill migration cho PR-8-era rows | 45m |
| 9.7 | Integration test (Supabase local) | 2h |
| 9.8 | Update spec + TODO.md | 30m |

**Total: ~9h dev effort.**

### Interface stability (PR-8 → PR-9)

`FindOrCreateTicketInput` unchanged. `FindOrCreateTicketOutput` **extended** (optional fields added):
...
`wire.ts` reads only `ticketId` → unaffected.
