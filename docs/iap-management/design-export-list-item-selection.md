...
| [`runAvailabilityReadPhase`](../../lib/iap-management/apple/availability-read-phase.ts) | ❌ **Do NOT reuse for the export fetch, and do NOT clone it — but NOT for the reason this row used to give.** ⚠ **CORRECTION.** An earlier draft called it *client-side*. **That is false.** Its only import is [`import type { AvailabilityForIap }`](../../lib/iap-management/apple/availability-read-phase.ts#L51) — a type-only import that erases at compile time — and every seam (`readOne`, `acquire`, `release`, `isCancelled`, `onProgress`) is injected. It runs server-side unchanged. **The real reason is the problem it solves.** It must hold a slot from the shared Hotfix-25 *client* queue, so its latch is checked **before claiming an index** ([:118-122](../../lib/iap-management/apple/availability-read-phase.ts#L118-L122)) — that pre-claim check is precisely what leaves the remainder genuinely unclaimed. Export's server path instead uses [`runStoppablePool`](../../lib/iap-management/stoppable-pool.ts) (shipped `9ff0c05`), which wraps `withConcurrency`: **claim-then-await**, with the remainder produced by the callback returning `skipped(item)`, not by leaving items unclaimed. Same three states, different mechanics — the trade-off is already written down in the [`stoppable-pool.ts` docstring, :41-48](../../lib/iap-management/stoppable-pool.ts#L41-L48); read that, not this row, for the authoritative version. ⚠ The old row's second clause ("✅ reused, unchanged, for the *opt-in availability filter* (G2 part 2)") is **also dead** — U3/U4 dropped that filter (implementation order 9), so there is no export-side reuse of this helper at all. Unifying the two pools is backlog: `[POOL-unify-availabilityReadPhase]` (PART 5). |
...
| **`[POOL-unify-availabilityReadPhase]`** — merging `runAvailabilityReadPhase` into `runStoppablePool` | Backlog, not this cycle. Both solve the same three-state (done / errored / never-attempted) problem, and neither is client-only — but under **different constraints**: the read phase must hold a slot from a shared *client* fetch queue, so it latches **before** claiming an index; `runStoppablePool` wraps `withConcurrency`, which **claims first** and lets the callback emit the remainder. Unifying means giving the pool a pre-claim hook — real work, zero user-visible change, and it would touch a shipped path (`bulk-availability`) for no behavioural gain. Constraint difference documented at [`stoppable-pool.ts:41-48`](../../lib/iap-management/stoppable-pool.ts#L41-L48). ⚠ **Now registered in `TODO.md`** (2026-08-26) — it lived here only until then, and the natural trigger recorded there is "the next time anyone edits `availability-read-phase.ts`, do this instead of patching in place". |
