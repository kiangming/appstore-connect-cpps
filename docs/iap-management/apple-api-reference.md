...
| Submit for review (LEGACY — kept intact for rollback) | POST | `/v1/inAppPurchaseSubmissions` | Relationship: `inAppPurchaseV2`. No attributes. Single-IAP submit route (`/submit`) GETs `/v2/inAppPurchases/{id}` first and 409s if state ≠ `READY_TO_SUBMIT`. Batch submit route (`/submit-batch` preflight + execute phases) refetches state via `listAllInAppPurchases` (fully paginated — MUST follow `links.next`; a single 200-item page silently truncated the live set on >200-IAP apps and produced false `NOT_FOUND` verdicts, an IAP.o.7a-class re-break), partitions via `partitionByStateGuard`, and skips non-ready rows with `SKIPPED_BY_STATE_GUARD` results (IAP.q.1.IV defence-in-depth). Both routes accept `?skipCheck=true` for explicit internal-caller bypass. Apple announced this endpoint deprecated 2026-07-15 (no sunset date) — the KB's [§4.11](IAP-MANAGEMENT-KNOWLEDGE-BASE.md#411-landmark--iap-submission-migrated-to-the-reviewsubmissions-mechanism-v2) LANDMARK notes the OpenAPI spec's `deprecated` flag does NOT reflect this — trust Apple's announcement, not the flag. |
| List IAP versions (v2 submit path) | GET | `/v2/inAppPurchases/{id}/versions` | Confirmed empirically: a `READY_TO_SUBMIT` IAP already has one version here, state `PREPARE_FOR_SUBMISSION` — read it, don't create one, in the common path. |
| Create IAP version (v2 submit path, rare fallback) | POST | `/v1/inAppPurchaseVersions` | Relationship: `inAppPurchase`. No writable attributes (Apple assigns `version`/`state` server-side). Only invoked when the list above returns no submittable version — expected rare-to-never given the empirical finding. |
| Find app's open reviewSubmission | GET | `/v1/apps/{appId}/reviewSubmissions?filter[platform]=IOS` | Used to create-or-reuse, NEVER blind-create — see the [§4.10](IAP-MANAGEMENT-KNOWLEDGE-BASE.md#410-landmark--cpp-and-iap-share-one-items-only-reviewsubmission-slot-per-app-platform) LANDMARK: CPP and IAP share one items-only slot per (app, platform). "Open" = any state except `COMPLETE`. |
| Create reviewSubmission | POST | `/v1/reviewSubmissions` | Body: `data.attributes.platform`, relationship `app`. Only called when the GET above finds none open. |
| Add item to reviewSubmission (IAP v2) | POST | `/v1/reviewSubmissionItems` | Relationships: `reviewSubmission` + `inAppPurchaseVersion` (sibling to CPP's `appCustomProductPageVersion` on the same request schema — confirmed against the OpenAPI spec's `ReviewSubmissionItemCreateRequest`). Paced 1000ms between items + `withRetry`/`AppleRateLimitError` on every call (the fix for the "52 items → 9 rate-limit failures" production bug). |
| List reviewSubmission items | GET | `/v1/reviewSubmissions/{id}/items` | Used by the Decision A conflict check to enumerate what's already in a reused (non-empty) submission before showing the conflict dialog. |
| Submit reviewSubmission (IAP v2) | PATCH | `/v1/reviewSubmissions/{id}` | Body: `data.attributes.submitted: true`. **Not** a `state` enum write — `state` is read-only/server-computed on this resource. Same shape CPP already uses. |
| Cancel/rollback reviewSubmission | DELETE | `/v1/reviewSubmissions/{id}` | Only safe on a FRESHLY-CREATED submission (`reused: false`) — deleting a REUSED submission would cancel items this flow never added (other IAPs, CPP pages). |

**v2 IAP submission mechanism** (the six rows above): full investigation, design, and the per-app `IAP_SUBMIT_V2_APPS` toggle are in [design-iap-v2-submission-migration.md](design-iap-v2-submission-migration.md). 200 items/reviewSubmission is Apple's official cap (per [Submit an In-App Purchase](https://developer.apple.com/help/app-store-connect/manage-submissions-to-app-review/submit-an-in-app-purchase/)), enforced client-side + a server zod `max(200)`.
