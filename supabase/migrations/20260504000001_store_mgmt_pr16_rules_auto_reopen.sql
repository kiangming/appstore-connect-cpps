-- ============================================================
-- Migration: Store Management — PR-16b.5 save/rollback rules thread
--                                  auto_reopen_eligible field
--
-- CREATE OR REPLACE three functions từ PR-16a.1 baseline (migration
-- 20260502000001) to read + write the new
-- subject_patterns.auto_reopen_eligible column added by
-- 20260504000000_store_mgmt_pr16_auto_reopen_eligible.
--
-- Mirrors PR-16a.1 auto_done_eligible threading exactly — both
-- fields now flow through:
--   1. build_rules_snapshot — emits auto_reopen_eligible alongside
--      auto_done_eligible trong snapshot JSONB
--   2. save_rules_tx — reads auto_reopen_eligible từ p_subject_patterns
--      payload, threads to INSERT
--   3. rollback_rules_tx — reads auto_reopen_eligible từ snapshot
--      JSONB với COALESCE(...,false) cho legacy snapshots predating
--      PR-16b.5
--
-- Backward compatibility:
--   - Pre-PR-16b.5 snapshots (config_snapshot.subject_patterns rows
--     lacking auto_reopen_eligible) replay with FALSE — no surprise
--     auto-reopen activation after rollback to historical version.
--   - Pre-PR-16b.5 save_rules_tx callers (none post-deploy because
--     the TS layer ships in same PR-16b.5 bundle) also receive FALSE
--     via same COALESCE.
--
-- Forward-only: this is the canonical save/rollback for PR-16b.5+.
-- Earlier 20260502000001 file kept untouched as historical record.
-- ============================================================

-- -----------------------------------------------------------------
-- build_rules_snapshot — include auto_reopen_eligible alongside
-- auto_done_eligible trong subject pattern object
-- -----------------------------------------------------------------
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'auto_reopen_eligible', auto_reopen_eligible
...
COMMENT ON FUNCTION store_mgmt.build_rules_snapshot IS
  'PR-16b.5: builds schema_version=1 snapshot. subject_patterns rows now include auto_done_eligible (PR-16a.1) AND auto_reopen_eligible (PR-16b.5). Both additive; legacy snapshots without either field replay as FALSE on rollback.';

-- -----------------------------------------------------------------
-- save_rules_tx — read auto_reopen_eligible từ p_subject_patterns
-- payload alongside auto_done_eligible
-- -----------------------------------------------------------------
...
BEGIN
  ...
  -- Insert subject patterns (PR-16a.1: + auto_done_eligible,
  -- PR-16b.5: + auto_reopen_eligible)
  FOR v_row IN SELECT * FROM jsonb_array_elements(p_subject_patterns) LOOP
    INSERT INTO store_mgmt.subject_patterns (
      ...
      auto_done_eligible, auto_reopen_eligible
    )
    VALUES (
      p_platform_id,
      v_row->>'outcome',
      ...
      COALESCE((v_row->>'auto_reopen_eligible')::BOOLEAN, false)
    );
  END LOOP;
...
COMMENT ON FUNCTION store_mgmt.save_rules_tx IS
  'PR-16b.5: thread auto_reopen_eligible (PR-16b.5) alongside auto_done_eligible (PR-16a.1) từ p_subject_patterns payload into the columns added by 20260502000000 + 20260504000000.';

-- -----------------------------------------------------------------
-- rollback_rules_tx — read auto_reopen_eligible từ snapshot,
-- default FALSE cho legacy snapshots
-- -----------------------------------------------------------------
...
BEGIN
  ...
  -- PR-16a.1: + auto_done_eligible. PR-16b.5: + auto_reopen_eligible.
  -- Pre-PR-16a.1 snapshots lack auto_done_eligible.
  -- Pre-PR-16b.5 snapshots lack auto_reopen_eligible.
  -- Both COALESCE to FALSE preserve opt-out default on rollback.
...
COMMENT ON FUNCTION store_mgmt.rollback_rules_tx IS
  'PR-16b.5: replay both auto_done_eligible (PR-16a.1) AND auto_reopen_eligible (PR-16b.5) từ snapshot. Pre-PR-16x snapshots default FALSE (opt-out preserved).';

-- ============================================================
-- END — 20260504000001_store_mgmt_pr16_rules_auto_reopen
-- ============================================================
