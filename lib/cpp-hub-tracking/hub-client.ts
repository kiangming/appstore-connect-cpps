/**
 * lib/cpp-hub-tracking/hub-client.ts — Server-side only.
 *
 * Thin REST client for the VNGGames Hub run-tracking API
 * (docs/integrate-rest-vnggames-hub.md). Ported from
 * lib/iap-management/hub-tracking/hub-client.ts byte-for-byte (kept as its
 * own copy per module-isolation convention, not a cross-import — see
 * docs/cpp-management/design-cpp-hub-tracking.md §2.G) — this file has no
 * CPP-specific logic at all.
 *
 * Every call is wrapped in a REAL AbortController + setTimeout hard abort —
 * a hung Hub call is aborted at HUB_TIMEOUT_MS regardless of what the
 * network is doing, so it can never hang the caller (the finalize route,
 * the dialog's validating→preview transition, or a Settings save).
 *
 ...
 *  TIMEOUT(3s) | FAILED <status> | ERROR <msg>. */
...
/** PATCH /runs/:id — best-effort. Never throws. Used for both the finalize
