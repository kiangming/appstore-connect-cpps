...
describe("hub-client (CPP)", () => {
  ...
  describe("hubStartRun", () => {
    it("returns the run id on success and sends workflow_id + actor", async () => {
      ...
      const result = await hubStartRun(
        { workflowId: "cpp-bulk-import", token: "tok", actor: "a@b.com" },
        1000,
        fetchImpl,
      );
      ...
      expect(bodyOf(fetchImpl)).toEqual({ workflow_id: "cpp-bulk-import", actor: "a@b.com" });
    });
  ...
  describe("hubCloseRun", () => {
    it("PATCHes the run with the terminal status", async () => {
      ...
      await hubCloseRun({ token: "tok", runId: "run-1", status: "SUCCESS" }, 1000, fetchImpl);
      expect(fetchImpl).toHaveBeenCalledWith(
