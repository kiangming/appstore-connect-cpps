/**
 * lib/cpp-hub-tracking/config.ts — Server-side only.
 *
 * Reads/writes the singleton VNGGames Hub tracking config row
 * (public.cpp_hub_tracking_config): workflow_id + encrypted ingest token +
 * the Settings `enabled` toggle. Encrypted-at-rest secret via the SAME
 * AES-256-GCM helpers ASC accounts use (no new crypto).
 *
 * Lives in `public` (CPP's existing schema, alongside asc_accounts) rather
 * than a dedicated `cpp_mgmt` schema like iap_mgmt/google_iap_mgmt use —
 * CPP predates that per-module schema-isolation convention. See
 * docs/cpp-management/design-cpp-hub-tracking.md §2.E.
 *
 * NO in-memory cache — deliberately, unlike lib/asc-account-repository.ts.
 * Ported from lib/iap-management/hub-tracking/config.ts, where an earlier
 * cached version caused two production bugs (stale "Token is required" on
 * blank-token save, and the `enabled` toggle appearing to "silently
 * revert" across a Railway rolling deploy's two-instances-briefly-side-by-
 * side window — commit 9ed7845). This table is read a handful of times per
 * bulk-import batch — nowhere near a hot path — so every read hits the DB.
 */
...
const TABLE = "cpp_hub_tracking_config";
