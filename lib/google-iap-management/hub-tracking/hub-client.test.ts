...
describe("hub-client", () => {
  ...
  describe("hubStartRun", () => {
    ...
    it("logs under the default tag when feature is omitted (Bulk Import behavior unchanged)", async () => {
      const fetchImpl = vi.fn(async () => jsonResponse(201, { id: "run-1" }));
      await hubStartRun({ workflowId: "wf", token: "tok" }, 1000, fetchImpl);
      expect(log.mock.calls.every((c) => c[0] === "google-iap-hub-tracking")).toBe(true);
    });

    it("logs under the given feature tag, never into the HTTP body sent to Hub", async () => {
      const fetchImpl = vi.fn(async () => jsonResponse(201, { id: "run-1" }));
      await hubStartRun(
        { workflowId: "wf", token: "tok", feature: "google-iap-bulk-activate" },
        1000,
        fetchImpl,
      );
      expect(log.mock.calls.every((c) => c[0] === "google-iap-bulk-activate")).toBe(true);
      expect(bodyOf(fetchImpl)).toEqual({ workflow_id: "wf" });
    });
  });
  ...
  describe("hubCloseRun", () => {
    ...
    it("logs under the given feature tag, never into the HTTP body sent to Hub", async () => {
      const fetchImpl = vi.fn(async () => jsonResponse(200, {}));
      await hubCloseRun(
        { token: "tok", runId: "run-1", status: "SUCCESS", feature: "google-iap-bulk-deactivate" },
        1000,
        fetchImpl,
      );
      expect(log.mock.calls.every((c) => c[0] === "google-iap-bulk-deactivate")).toBe(true);
      expect(bodyOf(fetchImpl)).toEqual({ status: "SUCCESS" });
    });
