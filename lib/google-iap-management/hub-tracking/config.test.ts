...
vi.mock("@/lib/google-iap-management/db", () => ({ googleIapDb: () => ({ from: fromMock }) }));
...
function makeRow(overrides: Partial<Row> = {}): Row {
  return {
    id: "default",
    workflow_id: "google-iap-bulk-import",
...
describe("no in-memory cache — every read hits the DB", () => {
  // Carried over from the Apple hub-tracking fix: an in-memory cache here
  // made a blank-token save wrongly see "no existing row" and made
  // `enabled` appear to silently revert across sessions/processes. This
  // table is read only a handful of times per bulk-import batch.
...
describe("saveHubTrackingConfig", () => {
  ...
  it("blank token + EXISTING token saved: succeeds, token_enc untouched, other fields updated", async () => {
    selectResult = { data: makeRow(), error: null };
    await saveHubTrackingConfig({
  ...
  it("enabled persists across a blank-token re-save (no cache, no revert)", async () => {
    ...
    // Re-save with a BLANK token — enabled must stay true, not reset.
    await saveHubTrackingConfig({
      workflowId: "wf",
      enabled: true,
      updatedBy: "a@b.com",
